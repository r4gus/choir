...
fn setup_test_db(conn: &PgConnection) {
    ...
    let a = NewAppointment {
        title: "iClass".to_string(),
        place: "Mimbergerstr. 52".to_string(),
        begins: NaiveDate::from_ymd(2021, 01, 16).and_hms(10, 58, 00),
        ends: NaiveDate::from_ymd(2021, 01, 16).and_hms(12, 58, 00),
        description: "This is a database test".to_string(),
    };

    let a2 = NewAppointment {
        title: "Singen".to_string(),
        place: "Mimbergerstr. 52".to_string(),
        begins: NaiveDate::from_ymd(2021, 3, 12).and_hms(10, 58, 00),
        ends: NaiveDate::from_ymd(2021, 4, 16).and_hms(12, 58, 00),
        description: "This is a database test".to_string(),
    };
    ...
    delete_all_appointments(conn);
    delete_all_participants(conn);
    create_user(&u1, conn);
    create_user(&u2, conn);
    create_user(&u3, conn);
    create_user(&u4, conn);
    ...
    create_appointment(&a, conn);
    create_appointment(&a2, conn);
}
...
#[test]
fn test_create_new_appointment() {
    let (conn, _) = make_connection_and_client();

    let a = NewAppointment {
        title: "First Appointment".to_string(),
        place: "Mimbergerstr. 52".to_string(),
        begins: NaiveDate::from_ymd(2021, 01, 16).and_hms(10, 58, 00),
        ends: NaiveDate::from_ymd(2021, 01, 16).and_hms(12, 58, 00),
        description: "This is a database test".to_string(),
    ...
    assert!(create_appointment(&a, &*conn).is_ok());
}

#[test]
fn test_get_all_appointments() {
    let (conn, _) = make_connection_and_client();
    ...
    let a2 = NewAppointment {
        title: "Second Appointment".to_string(),
        place: "Mimbergerstr. 52".to_string(),
        begins: NaiveDate::from_ymd(2021, 01, 16).and_hms(10, 58, 00),
        ends: NaiveDate::from_ymd(2021, 01, 16).and_hms(12, 58, 00),
        description: "This is a second database test".to_string(),
    };

    assert!(create_appointment(&a, &*conn).is_ok());
    assert!(create_appointment(&a2, &*conn).is_ok());
    assert_eq!(4, get_appointments(&*conn).unwrap().len());
}

#[test]
fn test_can_delete_appointments() {
    let (conn, _) = make_connection_and_client();
    ...
    assert!(create_appointment(&a, &*conn).is_ok());
    assert!(create_appointment(&a2, &*conn).is_ok());
    let apps = get_appointments(&*conn).unwrap();
    assert_eq!(4, apps.len());
    assert!(delete_appointment(apps[0].id, &*conn).is_ok());
    let apps2 = get_appointments(&*conn).unwrap();
    assert_eq!(3, apps2.len());
    assert_eq!(apps[1].id, apps2[0].id);
}
...
fn test_update_appointment() {
    ...
    let mut app = create_appointment(&a, &*conn).unwrap();
    app.title = "First Appointment (updated)".to_string();
    assert!(update_appointment(&app, &*conn).is_ok());
}
...
fn test_get_appointment() {
    ...
    create_appointment(&a, &*conn);
    let app = get_appointment_by_title("First Appointment", &*conn).unwrap();
    assert!(get_appointment(app.id, &*conn).is_ok());
}

#[test]
fn test_add_participent() {
    let (conn, _) = make_connection_and_client();

    let a = get_appointment_by_title("iClass", &*conn).unwrap();
    let a2 = get_appointment_by_title("Singen", &*conn).unwrap();
    let g = get_group_by_title("Bass", &*conn).unwrap();
    let u = get_user_by_mail("franzi@web.com", &*conn).unwrap();

    assert!(add_member_to_event(a.id, g.id, u.id, &*conn).is_ok());

    // can't add user to event twice
    assert!(add_member_to_event(a.id, g.id, u.id, &*conn).is_err());

    // can add user to different events with same group id
    assert!(add_member_to_event(a2.id, g.id, u.id, &*conn).is_ok());
}

#[test]
fn test_remove_participent() {
    let (conn, _) = make_connection_and_client();
    ...
    assert!(delete_participant(a2.id, u.id, &*conn).is_ok());
}
